export const portfolioObjOne = {
    label: '01',
    src: 'images/portfolio/MultiCountry.png',
    text: 'Multi-Country Paper Bill Scanner For Visually Impaired People',
    path: '/multicountry'
};

export const portfolioObjTwo = {
    label: '02',
    src: 'images/portfolio/Ropsten.png',
    text: 'Ropsten Ethereum Test Network and Crpyto Market',
    path: '/ropsten'

};

export const portfolioObjThree = {
    label: '03',
    src: 'images/portfolio/ProtectedRoutes.png',
    text: 'Protected Routes and User Roles using MERN Stack',
    path: '/protectedroutes'
};

export const portfolioObjFour = {
    label: '04',
    src: 'images/portfolio/PersonalWebsitePostGrad.png',
    text: 'Personal Website as College Graduate',
    path: '/pesonalwebsiteundergrad'
};


export const portfolioObjFive = {
    label: '05',
    src: 'images/portfolio/SimpleNotesMenu.png',
    text: 'Simple Notes using MERN Stack, JWT, REDUX',
    path: '/simplenotes'
};

export const portfolioObjSix = {
    label: '06',
    src: 'images/portfolio/ExpenseTrackerMenu.png',
    text: 'Expense Tracker using MERN Stack',
    path: '/expensetracker'

};

export const portfolioObjSeven = {
    label: '07',
    src: 'images/portfolio/DarkMode.png',
    text: 'Responsive Light Mode and Dark Mode React Js',
    path: '/darklighttheme'
};

export const portfolioObjEight = {
    label: '08',
    src: 'images/portfolio/PatientInfo.png',
    text: 'Patient Information System For SSCR Manila Clinic',
    path: '/patientinfo'
};

export const portfolioObjNine = {
    label: '09',
    src: 'images/portfolio/ConversionUndergrad.png',
    text: 'Conversion of My Personal Website as College Student to React.Js',
    path: '/personalwebsitecollegeconversion'
};

export const portfolioObjTen = {
    label: '10',
    src: 'images/portfolio/PersonalWebsiteUndergrad.png',
    text: 'My Personal Website as College Student',
    path: '/personalwebsitecollege'
};
